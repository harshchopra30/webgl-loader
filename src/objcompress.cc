  ...
  for (MaterialBatches::const_iterator iter = batches.begin();
    ...
    for (size_t i = 0; i < webgl_meshes.size(); ++i) {
      ...
      while (group_index < group_lengths.size()) {
		bool last = group_index  == group_lengths.size() - 1;
        fprintf(json_out,last ? "\"%s\" ": "\"%s\", ",
     ...
     time_t startCompress = time(NULL);

	webgl_loader::CompressAABBToUtf8(group_starts[group_index].bounds,
					 bounds_params, &sink);
     printf("time for compressing group : %d : %d\n",group_index,time(NULL) - startCompress );
      ...
      for (size_t k = 0; k < buffered_lengths.size(); ++k) {
		if(k == buffered_lengths.size() -1 )
			fprintf(json_out,PRIuS " ", buffered_lengths[k]);
		else
			fprintf(json_out,PRIuS ", ", buffered_lengths[k]);
      }

		const bool last =  (i ==webgl_meshes.size() - 1);
		if(last)
			fputs("]\n      }", json_out);
		else
			fputs("]\n      },", json_out);
